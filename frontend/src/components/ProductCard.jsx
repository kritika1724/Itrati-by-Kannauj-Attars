    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-[14px] border border-[rgba(25,33,60,0.08)] bg-[rgba(255,255,255,0.96)] shadow-[0_12px_28px_rgba(25,33,60,0.06)] transition duration-300 hover:-translate-y-1 hover:border-[rgba(200,169,106,0.42)] hover:shadow-[0_22px_64px_rgba(25,33,60,0.10)] sm:rounded-[28px] sm:shadow-[0_18px_52px_rgba(25,33,60,0.07)] sm:hover:-translate-y-1.5 sm:hover:shadow-[0_28px_80px_rgba(25,33,60,0.12)]">
      <div className="relative p-1.5 sm:p-4">
        <Link
          ...
          className="block overflow-hidden rounded-[12px] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(249,243,233,0.98))] sm:rounded-[22px]"
        >
        ...
        <div className="absolute left-2 top-2 z-10 flex max-w-[calc(100%-4rem)] flex-wrap gap-1 sm:left-6 sm:top-6 sm:max-w-[calc(100%-8rem)] sm:gap-2">
          {badges.map((badge) => (
            <span
              key={badge}
              className={`rounded-full px-1.5 py-0.5 text-[7px] font-semibold uppercase tracking-[0.1em] shadow-sm sm:px-3 sm:py-1 sm:text-[10px] sm:tracking-[0.24em] ${
                badge === 'Bestseller'
        ...
        <div className="absolute right-2 top-2 z-10 flex items-center gap-1 sm:right-6 sm:top-6 sm:gap-2">
      ...
      <div className="flex flex-1 flex-col px-2 pb-2.5 pt-0 sm:px-6 sm:pb-6 sm:pt-1">
        <div className="flex items-start justify-between gap-2.5">
          <div>
            <p className="text-[8px] font-semibold uppercase tracking-[0.12em] text-[#8D7667] sm:text-xs sm:tracking-[0.24em]">{product?.category || 'Attar'}</p>
            <Link to={productPath} state={{ productId: product?._id }} className="mt-1 block sm:mt-2">
              <h3 className="line-clamp-2 text-[0.82rem] font-semibold leading-[1.15] text-[#19213C] sm:text-2xl sm:leading-tight">
                {product.name}
              </h3>
            </Link>
          </div>
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-1.5 sm:mt-5 sm:gap-3">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 sm:mt-2 sm:gap-2">
              <span className="text-[0.95rem] font-semibold text-[#19213C] sm:text-2xl">₹{Number(price || 0).toLocaleString('en-IN')}</span>
              {minPack?.onSale ? (
                <span className="text-xs text-[#8E8E96] line-through sm:text-sm">₹{Number(minPack.price || 0).toLocaleString('en-IN')}</span>
              ) : null}
            </div>
            {minPack?.label ? <p className="mt-0.5 text-[9px] text-[#6B6F7A] sm:mt-1 sm:text-sm">{minPack.label}</p> : null}
          </div>
        ...
        {sampleEnabled ? (
          <button
            ...
            className="mt-2 rounded-[12px] border border-[rgba(200,169,106,0.22)] bg-[rgba(255,250,244,0.96)] px-2 py-1.5 text-left transition hover:border-[rgba(200,169,106,0.38)] sm:mt-4 sm:rounded-[18px] sm:px-4 sm:py-3"
          >
        ...
        {showActions ? (
          <div className="mt-auto grid grid-cols-2 gap-1.5 pt-2.5 sm:gap-3 sm:pt-6">
            {!isAdmin ? (
              <button
                type="button"
                onClick={() => onAdd?.()}
                className="rounded-full bg-[linear-gradient(135deg,#E9D28A_0%,#C9A24A_100%)] px-1.5 py-2 text-[10px] font-semibold leading-tight text-[#1B233F] shadow-[0_14px_28px_rgba(196,139,106,0.2)] transition hover:translate-y-[-1px] sm:px-5 sm:py-3 sm:text-sm sm:shadow-[0_18px_40px_rgba(196,139,106,0.24)]"
              >
                Add to Cart
              </button>
            ) : (
            ...
            <button
              ...
              className="rounded-full border border-[rgba(25,33,60,0.12)] bg-white px-1.5 py-2 text-[10px] font-semibold text-[#19213C] transition hover:border-[rgba(200,169,106,0.38)] sm:px-5 sm:py-3 sm:text-sm"
            >
              Details
            </button>
          </div>
